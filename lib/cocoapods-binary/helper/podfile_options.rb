  ...
  class Podfile
    class TargetDefinition
      ...
      def prebuild_framework_pod_names
        ...
        names += parent.prebuild_framework_pod_names if !parent.nil? && parent.is_a?(TargetDefinition)
        names
      end

      def should_not_prebuild_framework_pod_names
        names = @should_not_prebuild_framework_pod_names || []
        names += parent.should_not_prebuild_framework_pod_names if !parent.nil? && parent.is_a?(TargetDefinition)
        names
      end
...
module Pod
  class Installer
    ...
    def validate_every_pod_only_have_one_form
      multi_targets_pods = pod_targets.group_by(&:pod_name).select do |_k, v|
        v.map { |t| t.platform.name }.count > 1
      end

      multi_targets_pods = multi_targets_pods.reject do |_name, targets|
        contained = targets.map { |t| prebuild_pod_targets.include? t }
      ...
      warnings += multi_targets_pods.map do |name, targets|
        "         #{name}: #{targets.map do |t|
                               t.platform.name
                             end }"
      end.join("\n")
