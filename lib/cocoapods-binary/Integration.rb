  ...
  class Installer
    class PodSourceInstaller
      def install_for_prebuild!(standard_sanbox)
        return if standard_sanbox.local? name
        ...
        def walk(path, &action)
          ...
          path.children.each do |child|
            ...
            walk(child, &action) if child.directory? && result
          end
        end

        def make_link(source, target)
          return if ['.dSYM'].include?(source.extname)
        ...
        target_names.each do |name|
          ...
          walk(real_file_folder) do |child|
            ...
            if child.directory? && %w[.framework .dSYM].include?(child.extname)
  ...
  class Installer
    ...
    define_method(:resolve_dependencies) do
      ...
      def empty_source_files(spec)
        ...
        %w[ios watchos tvos osx].each do |plat|
          spec.attributes_hash[plat]['source_files'] = [] unless spec.attributes_hash[plat].nil?
        end
      end

      specs = analysis_result.specifications
      prebuilt_specs = (specs.select do |spec|
        prebuild_pod_names.include? spec.root.name
      end)
    ...
    define_method(:install_source_of_pod) do |pod_name|
      ...
      if prebuild_pod_names.include? pod_name
